...
- **Preview:** follow [preview.md](preview.md). Each prompt begins with a defined image type, includes a specific era and region, uses a defined artistic style or clear color treatment, and forms one concise paragraph without field labels or texture/shadow microdetail. Each candidate is one person-led image sharing the representative content baseline while testing a distinct direction compatible with the screenplay. Flag prompt pollution, irrelevant off-frame detail, conflicting world facts, tonal drift, generic AI polish, or a result that cannot guide production assets.
