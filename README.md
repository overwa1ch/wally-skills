...
| `wally-static-asset-designer` | V2.12 | 提示词请求交付文本；需要图片时通过内置 Browser 建立命名聊天，每项资产独立聊天；Preview 至少 8 套不同风格提示词与图像，各项资产至少 4 套不同提示词与图像，逐版生成供用户选择，并继承已选全局效果。 |
...
- `wally-static-asset-designer` 的 Preview 提示词仅由“图片类型定义＋文化锚点＋视觉风格参照＋画面描述”依次组成一个自然段。参照可来自创作者、作品、影视传统与类型、艺术流派、摄影与成像风格；通常一个主参照即可，不附创作年代，不默认展开摄影参数、细碎调色、纹理或暗部。
