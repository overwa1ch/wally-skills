...
- Static V2.12 将 Preview 正文固定为图片类型定义、文化锚点、视觉风格参照、画面描述，按顺序连成一个自然段。
- 视觉参照涵盖创作者、具体作品、影视传统与类型、艺术流派与表现传统、摄影与成像风格；允许直接使用王家卫、韦斯·安德森、富士等名称，以更换主参照测试整体效果，不强制拆解成摄影与调色参数。同步模板、规划、审查及风格路由，保留已确认构图与世界事实。

- Static V2.11 同步已确认的 Preview 写法：先定义图片类型，写明具体故事年代与地域，用一个简短自然段表达明确调色或已有定义的艺术风格；艺术参照不附创作年代，不展开纹理、暗部等微观细节。
- Preview 风格探索覆盖影视传统、导演风格或艺术流派；仅在用户限定调色测试时收窄为配色变化。入口、模板、规划与审查规则同步，通用 craft 的展开顺序与细节建议不再应用于 Preview；候选数、正式资产类型与固定绘图提示词保持。

- Action V3.25 灵活使用 J-cut／L-cut，将具体声音时序与跨镜衔接写入相关 Shot，并保留原 Audio 整体设定。
- Reference 统一承载对象形象，有图时减少对图片已传递信息的文字复述，无图时直接写必要的文字形象设定。
- Constraints 仅保留有明确上下文和实际控制作用的限制，清除无用或脱离场景语境的否定。其他设计规则与输出结构保持。

## 2026-09-21

- Action V3.24 恢复用户确认的提示词写法：沿用 `Style:`、`Reference:`、`Camera:`、`Acting:` 等英文标签，按需选择字段；Shot 以初始画面和连续动作正文展开，连续变化按实际节拍组织。保留按场景完整设计、场内连续计时、原镜号、已有有效字段及已确认内容。
