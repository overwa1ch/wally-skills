...
- 发布 9 月 14 日完成的工作树修订：Storyboard V3.20 的成品工具支持逐页选定候选，允许不同页采用不同版本；未选或拒绝的候选不阻塞已选成品。比较全部版本需明确指定，裁切仍遵守原确认边界。
- Static V2.10 将提示词请求限定为文本交付；Helper V4.3 的流程参考将视觉测试作为建议，保留用户已有批准状态，修正资产类型登记与生成交接。
- Screenplay V2.2 把分步开发、审批菜单和检查点下沉到按需参考；Visual V1.7 的普通解释直接回答，结构化任务只读取对应模板章节。专业方法、候选数、资产几何与固定绘图提示词保持原样。

## 2026-09-19

- Helper 升级为 V4.3：移除 Route A/B、外层提示词模板、绑定清单格式及配套校验器、测试与 fixtures。保留材料整理、流程经验、按需兼容审查、专业技能交接和固定 BGM 请求；视频提示词与参考绑定使用 Action 的现行产物。同步模块职责、README、健康检查和 CI。

- `story-idea-generator` V1.2 将万字纲方法更新为四幕十序列：幕是路标之间的结构区间，事件是序列，结局在四幕之外单列。同步十个事件的作用、八属性矩阵与过激动作的位置，保持两阶段边界和按需读取。

- `story-idea-generator` V1.1 负责故事支点与万字纲两阶段，增加“拿事儿说话”的详纲方法、三幕／四幕事件数轴与八要素矩阵；共同基础、两阶段方法、电影案例和模板分别按需读取。完整剧本继续由编剧技能负责。

- 新增 `story-idea-generator` V1.0，保留明确点名调用，以及入口、构思方法、电影案例的渐进式披露。
- 故事灵感、剧本、分镜脚本分别由对应技能负责；更新 Helper 能力入口、安装命令与七项技能健康检查。已有材料可直接进入相应阶段。

## 2026-09-13

- GitHub CI 安装文泉驿中文 TrueType 字体，审查拼图与 PDF 支持对应 Linux 字体路径，修复 Linux 缺少中文字体导致的回归失败。
- Storyboard 升级为 V3.19：按已有场景提取完整材料及适用的全局设定。故事板每场一个独立聊天；分镜表在基础聊天一次上传全部静态资产后，每场各建一个命名分支，同场多页留在该聊天。表演设计等同导演脚本。先交付原图，裁切和成品制作经确认后执行；可选工具记录场景聊天、共享资产与逐镜原文，并支持自包含图文 HTML。
- Action 升级为 V3.23：按场景完成整场戏，镜号保留，时间戳每场从 0.0s 开始且场内连续。取消单镜头／多镜头格式分流及统一 Shot、固定字段顺序、双层时间码；保留已有有效内容，正文直接描述表演及视听关系。
- 渐进披露整理：入口只维护范围与路由，生成流程与确认后的成品流程分开读取；表演设计分为交付约定、设计方法和回片审查。以上替代此前未确认的 V3.18／V3.22 本地方案。

## 2026-09-12

- Screenplay 升级为 V2.1：采用“如果你不确定，就写得更少，而不是更多”；作品定义改为可选信息，缺项省略，删除无作用的用途标签；已有稿整理和局部修改直接交付，分步审批按用户选择启用。短叙事按真实体量缩放，时长分析按任务需要展开。

- 六项 Wally skill 统一改为明确点名调用，`agents/openai.yaml` 设置 `policy.allow_implicit_invocation: false`，入口说明与安装文档同步收紧触发范围。点名专业 skill 只启用该 skill；专业方法、模板和模块版本保持原值。
